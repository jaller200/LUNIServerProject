						...
						switch (packet->data[3]) {
							...
							{ 
								...
								if (zone == NIMBUS_ISLE)
									...
									//This tries to test the ability to travel to NS w/ a rocket...
									v = OpenPacket(".\\world\\test\\reckettest.bin");
								}
							...
							case CLIENT_GENERAL_CHAT_MESSAGE:
							{
									...
									bitStream.Write(l1);
									bitStream.Write(l2);
									bitStream.Write(wMsg.length() * 2 + 2);
									bitStream.Write(l2);
									bitStream.Write(l3);
							...
							case 5:
							{
								...
								if (packet->data[0x1A] == 123 && packet->data[0x1B] == 33 & packet->data[0x1C] == 22) {
									...
									ZoneId zone = NIMBUS_STATION;

									auto v = OpenPacket(".\\world\\test\\rockettest.bin");
									ServerSendPacket(rakServer, v, packet->systemAddress);
								}
							}
								break;
