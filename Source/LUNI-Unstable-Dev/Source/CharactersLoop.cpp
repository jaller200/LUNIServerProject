...
void CharactersLoop(CONNECT_INFO* cfg, Ref< UsersPool > OnlineUsers, Ref< CharactersPool > OnlineCharacters, Ref< CrossThreadQueue< string > > OutputQueue) {
					...
					case WORLD:
						...
						switch (packet->data[3]) {
							...
							case CLIENT_LOGIN_REQUEST:
							{
								...
								ulonglong characterID;
								memcpy(&characterID, packet->data + 0x08, 8);

								auto newChar = Character::InitializeCharacter(characterID);
								OnlineCharacters->Insert(newChar, packet->systemAddress);
								...
								//user->current = Character::InitializeCharacter(characterID);
